...
void CSocket_Recv::TakeApartPack(char *szReceive, int nLength)
{
	...
	while(1)
	{
		...
		DBPacket *pak;
		pak=(DBPacket*)szPack;
		RunLog("DBGATE=>RECVPAK[%d][%s]",pak->PackType,GetDBPakName(pak->PackType));
		DisplayPack(szPack);
		ParsePacket(szPack);		
	}
}
...
/*						if(m_db.m_ch[chan]._rs != NULL)
				...
				if(m_db.m_ch[chan].SQL.find("select ")+m_db.m_ch[chan].SQL.find("update ")+m_db.m_ch[chan].SQL.find("delete ")+m_db.m_ch[chan].SQL.find("create ")+m_db.m_ch[chan].SQL.find("drop ")+m_db.m_ch[chan].SQL.find("insert ") ==-6)
					m_db.m_ch[chan].iCmdType = 0;
				else m_db.m_ch[chan].iCmdType=1;
		...
		case PAK_SQLEXEC:
			{
				...
				int s = m_db.m_ch[chan]._rs->State;
...
void CSocket_Recv::ConnectServer()
{
	...
	UINT nPort = GetPrivateProfileInt("tcp","port",0,"./DBGate.ini");
	GetPrivateProfileString("tcp", "ip","", szIP, sizeof(szIP),"./DBGate.ini");
...
void CSocket_Recv::SendToServer(DBPacket &pak)
{
	...
	RunLog("DBGATE=>SENDPAK[%d][%s]",pak.PackType,GetDBPakName(pak.PackType));
}
