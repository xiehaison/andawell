...
void CSocket_Recv::ConnectServer()
{
	...
	GetPrivateProfileString("tcp", "ip","", szIP, sizeof(szIP),CONFIG_FILE);
...
*/
void CSocket_Recv::SendToServer(DBPacket &pak,int node)
{
    int nSize = m_db.m_ch.size();
    ...
    //
    int len = SendPacket(node, (char *)&pak, sizeof(pak));
    if (pak.Result == 1)
...
//备注：所有发送到服务器的TCP包,都要调用此函数
//*/
//void CSocket_Recv::SendToServer(T_MsgServer &msg)
//{
//    DBPacket pak = (DBPacket &)msg.msg;
//    int node = msg.node;
//    int nSize = m_db.m_ch.size();
//
//    CListCtrl * pList = p_ListRecv;// p_ListSend;//
